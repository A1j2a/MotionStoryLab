          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[11px] text-[#FF6B00] hover:text-[#EA580C] flex items-center gap-1 font-semibold cursor-pointer"
              >
                {copiedSection === "description" ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3 text-[#FF6B00]" />}
                <span>{copiedSection === "description" ? "Description Copied!" : "Copy Description"}</span>
              </button>
            </div>
            <textarea
              rows={6}
              ...
              placeholder="Full YouTube description with chapters, lyrics and hashtags..."
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#FAFAFC] border border-[#E5E5EA] text-xs font-mono text-[#1D1D1F] focus:outline-none focus:border-[#FF6B00] leading-relaxed"
            />
          </div>

          {/* Video Chapters / Timestamps Card with Copy Button */}
          <div className="p-3.5 rounded-2xl bg-[#FAFAFC] border border-[#E5E5EA] space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-[#1D1D1F] flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5 text-blue-600" />
                <span>Video Chapters & Timestamps ({pkg.chapters?.length || 0})</span>
              </span>
              <button
                type="button"
                onClick={() => {
                  const chText = pkg.chapters?.map((c) => `${c.time} - ${c.title}`).join("\n") || "";
                  copyToClipboard(chText, "chapters");
                }}
              ...
              >
                ...
                <span>{copiedSection === "chapters" ? "Chapters Copied!" : "Copy Chapters"}</span>
              </button>
            </div>
            <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
              {pkg.chapters && pkg.chapters.length > 0 ? (
                pkg.chapters.map((c, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-xs">
                    <input
                      type="text"
                      value={c.time}
                      onChange={(e) => {
                        const next = [...(pkg.chapters || [])];
                        next[idx] = { ...next[idx], time: e.target.value };
                        setPkg({ ...pkg, chapters: next });
                      }}
                      className="w-16 px-2 py-1 rounded-lg bg-white border border-[#E5E5EA] font-mono text-[11px] text-blue-700 font-semibold text-center"
                    />
                    <input
                      type="text"
                      value={c.title}
                      ...
                      className="flex-1 px-2.5 py-1 rounded-lg bg-white border border-[#E5E5EA] text-xs text-[#1D1D1F]"
                    />
                  </div>
                ))
              ) : (
                <p className="text-[11px] text-[#86868B] italic">No chapters generated.</p>
              )}
            </div>
          </div>
