...
export function StoryboardView({
  ...
  const handleRerender = async (sceneId: string, sceneNum: number) => {
    ...
    try {
      await api.rerenderScene(projectId, sceneId);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-[#E5E5EA]">
        <div className="space-y-1">
          ...
          <p className="text-xs text-[#6E6E73]">
            Each 5–10s shot is synchronized to the approved lyrics and audio timeline. Individual scenes can be regenerated independently.
          </p>
        </div>
            ...
            >
              ...
              <div className="pt-2 border-t border-[#EDEDF0] flex items-center justify-between gap-2">
                <button
                  ...
                  className="w-full inline-flex items-center justify-center gap-1.5 bg-white hover:bg-orange-50 hover:text-[#FF6B00] text-[#1D1D1F] border border-[#E5E5EA] text-[11px] font-bold py-2 rounded-xl transition-all cursor-pointer"
                >
                  {isRendering ? (
                    <>
